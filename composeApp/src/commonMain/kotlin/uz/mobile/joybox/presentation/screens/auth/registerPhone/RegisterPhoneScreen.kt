package uz.mobile.joybox.presentation.screens.auth.registerPhone
...
import joybox.composeapp.generated.resources.your_phone_number
import joybox.composeapp.generated.resources.your_phone_number_description
...
import uz.mobile.joybox.presentation.components.PrivacyPolicyAgreement
import uz.mobile.joybox.presentation.platform.MoviesAppScreen
import uz.mobile.joybox.presentation.screens.auth.registerOtp.RegisterOtpScreen
import uz.mobile.joybox.presentation.screens.naviagationActions.navigateToOfertaScreen
...
class RegisterPhoneScreen(val userData: User) : MoviesAppScreen {
    ...
    @Composable
    fun RegisterPhoneScreenContent(
        navigator: Navigator = LocalNavigator.currentOrThrow,
        viewModel: RegisterPhoneViewModel = koinScreenModel(),
        ...
        LaunchedEffect(phoneState) {
            if (phoneState is PhoneState.Success) {
                navigator.push(RegisterOtpScreen(userData = userData.copy(phone = phoneState.phone)))
        ...
        key(languageViewModel.selectedLanguage) {
            Column(
                ...
                PrivacyPolicyAgreement(isChecked = viewModel.isAgree, onCheckedChange = viewModel::updateAgreement) {
                    navigateToOfertaScreen(navigator)
                }

                Spacer(Modifier.fillMaxWidth().height(20.dp))

                MainButton(modifier = Modifier.height(41.dp),text = stringResource(Res.string.continuee), onClicked = viewModel::onNext, isEnable = viewModel.isAgree, isLoading = (phoneState is PhoneState.Loading))
